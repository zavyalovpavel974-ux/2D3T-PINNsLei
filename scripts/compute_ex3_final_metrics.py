"""Compute Ex3 metrics with swapped_xy coordinate convention."""
...
def main():
    ...
    for time_label, ref_file in REFERENCE_FILES.items():
        ...
        # Swapped coordinates: (y, x, t)
        X = np.column_stack([y_mesh.reshape(-1), x_mesh.reshape(-1), np.full(nx * ny, t_val)])
        with torch.no_grad():
        ...
        print(f"  t={time_label}: Te L2={time_metrics['Te']['L2']:.4e}, Ti L2={time_metrics['Ti']['L2']:.4e}, Tr L2={time_metrics['Tr']['L2']:.4e}")
    ...
    metrics = {
        ...
        "coordinate_evaluation": "swapped_xy",
    ...
    output_json = "runs/ex3_transfer_clean_20260528/reports/example3_metrics.json"
    os.makedirs(os.path.dirname(output_json), exist_ok=True)
    with open(output_json, "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)
    print(f"\n[ok] wrote {output_json}")
    print(f"\nAggregate (swapped_xy):")
    for name in ("Te", "Ti", "Tr"):
        print(f"  {name}: L2={aggregate[name]['L2']:.4e}, L1={aggregate[name]['L1']:.4e}, Linf={aggregate[name]['Linf']:.4e}")

    # Paper comparison
    paper = {
        "Te": {"L2": 8.730e-3, "L1": 2.471e-3, "Linf": 6.884e-3},
        "Ti": {"L2": 1.103e-2, "L1": 2.536e-3, "Linf": 6.175e-3},
        "Tr": {"L2": 3.349e-3, "L1": 2.406e-3, "Linf": 1.301e-2},
    }
    print(f"\nPaper Table 6 comparison:")
    for name in ("Te", "Ti", "Tr"):
        ratio = aggregate[name]['L2'] / paper[name]['L2']
        print(f"  {name} L2: ours={aggregate[name]['L2']:.4e}, paper={paper[name]['L2']:.4e}, ratio={ratio:.2f}x")
